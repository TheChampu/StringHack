...
async def e(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        grp_name = await ask_id(
            x, text='ᴘʟᴇᴀsᴇ sᴇɴᴅ ɢʀᴏᴜᴘ/ᴄʜᴀɴɴᴇʟ ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ɪɴᴠɪᴛᴇ ʟɪɴᴋ')
        if not grp_name:
            return
...
async def f(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        if not string:
            return
        grp_name = await ask_id(
            x, text='ᴘʟᴇᴀsᴇ sᴇɴᴅ ɢʀᴏᴜᴘ/ᴄʜᴀɴɴᴇʟ ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ɪᴅ')
        if not grp_name:
            return
...
async def k(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        text = f"**ᴛᴏᴛᴀʟ ɢʀᴏᴜᴘ/ᴄʜᴀɴɴᴇʟ: __{len(dialogs)}__\nᴇxᴘᴇᴄᴛᴇᴅ ᴛɪᴍᴇ: __{len(dialogs)*1.5}__ sᴇᴄᴏɴᴅs**"
        msg = await x.send_message(text)
        result = "**sᴜᴄᴄᴇssғᴜʟʟʏ ʟᴇғᴛ: __{}__\nʀᴇᴍᴀɪɴɪɴɢ ᴄʜᴀᴛs: __{}__**\n\nᴛʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ ᴛʜɪs ʙᴏᴛ"
...
async def l(e):
    async with bot.conversation(e.chat_id) as x:
        await x.send_message("ᴄʜᴏᴏsᴇ ᴛʜᴇ ʙʀᴏᴀᴅᴄᴀsᴛ ᴛʏᴘᴇ",
...
async def _123(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        TEXT = f"**ᴛᴏᴛᴀʟ ᴄʜᴀᴛs: __{len(dialogs)}__\nᴇxᴘᴇᴄᴛᴇᴅ ᴛɪᴍᴇ: __{len(dialogs)*2}__ sᴇᴄᴏɴᴅs**"
